...
export default function BookingScreen() {
  ...
  // ==========================================
  // 🔴 TODO: GANTI GAMBAR SLIDER (CAROUSEL) DI SINI
  // ==========================================
  const headerImages = [
    'https://images.unsplash.com/photo-1520250497591-112f2f40a3f4?q=80&w=2070&auto=format&fit=crop', 
    'https://images.unsplash.com/photo-1544552866-d3ed42536cfd?q=80&w=2071&auto=format&fit=crop', 
  ...
  // 🔴 TODO: GANTI DATA PERALATAN DI SINI
  const [equipment, setEquipment] = useState([
    { id: 1, name: 'Joran Pancing', price: 25000, count: 0 },
    { id: 2, name: 'Kail Pancing', price: 10000, count: 0 },
    { id: 3, name: 'Umpan Ikan', price: 15000, count: 0 },
    { id: 4, name: 'Jaring Kecil', price: 20000, count: 0 },
  ]);

  // Hitung Harga
  useEffect(() => {
    const BASE_PRICE_PER_HOUR = 15000; 
  ...
  const formatDate = (rawDate: Date) => {
    return rawDate.toLocaleDateString('id-ID', { day: '2-digit', month: '2-digit', year: 'numeric' });
  };
  ...
  const updateEquipment = (id: number, type: 'plus' | 'minus') => {
    const newData = equipment.map((item) => {
      if (item.id === id) {
        ...
        if (type === 'minus' && item.count > 0) return { ...item, count: item.count - 1 };
      }
      return item;
    });
  ...
  // ==========================================
  return (
    // 2. GANTI View JADI GestureHandlerRootView BIAR BISA SWIPE
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.sectionContainer}>
                 ...
                 <View style={styles.webInputOverlay}>
                    {React.createElement('input', {
                        ...
                        style: {
                            ...
                            opacity: 0, cursor: 'pointer', zIndex: 100
                        }
                    })}
                 </View>
              ) : (
           ...
           <View style={styles.inputWrapper}>
              ...
              <View style={{flexDirection: 'row', alignItems: 'center'}}>
                 <View style={styles.spinnerContainer}>
                    <TouchableOpacity onPress={() => updateDuration('plus')} style={styles.spinnerUp}>
                       <Ionicons name="caret-up" size={12} color="#555" />
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => updateDuration('minus')} style={styles.spinnerDown}>
                       <Ionicons name="caret-down" size={12} color="#555" />
                    </TouchableOpacity>
                 </View>
                 {/* 🔴 TODO: GANTI ICON JAM */}
                 <Ionicons name="time-outline" size={20} color="#1F2937" style={{marginLeft: 10}} />
              </View>
           </View>
           ...
           <View style={styles.inputWrapper}>
              ...
              <View style={{flexDirection: 'row', alignItems: 'center'}}>
                 <View style={styles.spinnerContainer}>
                    <TouchableOpacity onPress={() => updatePeople('plus')} style={styles.spinnerUp}>
                       <Ionicons name="caret-up" size={12} color="#555" />
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => updatePeople('minus')} style={styles.spinnerDown}>
                       <Ionicons name="caret-down" size={12} color="#555" />
                    </TouchableOpacity>
                 </View>
                 {/* 🔴 TODO: GANTI ICON ORANG */}
                 <Ionicons name="people-outline" size={20} color="#1F2937" style={{marginLeft: 10}} />
              </View>
           </View>
        </View>
           ...
           <View style={styles.gridContainer}>
              {equipment.map((item) => (
                <View key={item.id} style={styles.card}>
                   ...
                   {/* 🔴 TODO: GANTI GAMBAR PERALATAN */}
                   <Image 
                     source={{ uri: 'https://img.freepik.com/free-vector/fishing-equipment-concept_1284-13233.jpg' }} 
                   ...
                   <View style={styles.cardContent}>
                     ...
                     <View style={styles.counterContainer}>
                        <TouchableOpacity style={styles.counterBtn} onPress={() => updateEquipment(item.id, 'minus')}>
                           <Text>-</Text>
                        </TouchableOpacity>
                        <Text style={styles.counterText}>{item.count}</Text>
                        <TouchableOpacity style={styles.counterBtn} onPress={() => updateEquipment(item.id, 'plus')}>
